...
function __generateThumbnailFunction(url, tab) {
    return function() {
        __generateThumbnail(url, tab);
    };
}

function __generateThumbnailAndCloseFunction(url, tab) {
    return function() {
        __generateThumbnail(url, tab);
        __closeTab(tab);
    };
...
function __processTab(url) {
    return function(tab) {
        console.log("__applyThumbnailStyleToTab");
        __applyThumbnailStyleToTab(tab, url);

        const firstThumbnailTimeout = timers.setTimeout(
            __generateThumbnailFunction(url, tab),
            constants.FIRST_THUMBNAIL_DELAY
        );

        const secondThumbnailTimeout = timers.setTimeout(
            __generateThumbnailFunction(url, tab),
            constants.SECOND_THUMBNAIL_DELAY
        );

        const thirdThumbnailTimeout = timers.setTimeout(
            __generateThumbnailAndCloseFunction(url, tab),
            constants.THIRD_THUMBNAIL_DELAY
        );

        tab.on("close", function() {
            timers.clearTimeout(firstThumbnailTimeout);
            timers.clearTimeout(secondThumbnailTimeout);
            timers.clearTimeout(thirdThumbnailTimeout);
        });
    };
}
